...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  w_start_idx = data_index
  w_end_idx = w_start_idx + window_size - 1

  while ((batch_count < batch_size) and (w_end_idx < len(data))):
    w_context_idx = (w_start_idx + w_end_idx)//2
    left = 1
    right = 1
    direction = 1
    for x in range(num_skips):
      batch[batch_count] = data[w_context_idx]
      if direction % 2 == 1:
        labels[label_idx] = data[w_context_idx - left]
        left += 1
      else:
        labels[label_idx] = data[w_context_idx + right]
        right += 1
      direction += 1
      label_idx +=1
      batch_count += 1
...
def build_model(sess, graph, loss_model):
  ...
  with graph.as_default():
    ...
    with tf.device('/cpu:0'):
      ...
      #This represent the 128 length vector for each word ina vocab, each word represented by row
      #This is the representation for center words
      embeddings = tf.Variable(
          tf.random_uniform([vocabulary_size, embedding_size], -1.0, 1.0))

      # This represent 128 length vector for each word in train_inputs by taking out 
      # the coreesponding row from embeddings
      embed = tf.nn.embedding_lookup(embeddings, train_inputs)

      #This represent the 128 length vector for each word in vocab, each word represented by row
      #This is the representation for predicted words
...
def train(sess, model, data, dictionary, batch_size, num_skips, skip_window, 
  ...
  for step in xrange(max_num_steps):
    ...
    # x = tf.Print(batch_inputs,[batch_inputs],message="DEBUG:A")
    # print("=========================")
    # sess.run(x)
